...
如`tmux -CC`命令会开启一个新的 tmux 会话，并启用控制模式，之后你的 iterm 会打开一个新的窗口，用于展示 tmux 会话；该窗口的表现和没使用 tmux 时一样，你不需要通过快捷键或 tmux 命令来新建多窗口和多窗格。使用 iterm2 的快捷键`Command+D`和`Command+Shift+D`来分隔窗格，使用`Command+N`来新建 Tab，使用`Command+N`来新建窗口。
...
[^1]: [tmux | 我的学习笔记](./2021-11-27-tmux-a-terminal-multiplexer.md)
